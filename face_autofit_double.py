...
logger = logging.getLogger("FaceAutoFitDouble")
...
class FaceAutoFitDouble:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "mask_R": ("MASK",),    # 右侧人物蒙版
                "mode": (["half_body", "full_body", "custom"], {"default": "half_body"}),
                ...
                "size_L": ("FLOAT", {"default": 1.0, "min": 0.5, "max": 2.0, "step": 0.05}),
                "size_R": ("FLOAT", {"default": 1.0, "min": 0.5, "max": 2.0, "step": 0.05}),
            ...
            "optional": {
                "custom_width": ("INT", {"default": 760, "min": 256, "max": 4096, "step": 64}),
                "custom_height": ("INT", {"default": 1024, "min": 256, "max": 4096, "step": 64}),
            }
        }

    RETURN_TYPES = ("IMAGE", "MASK", "MASK", "MASK")
    RETURN_NAMES = ("combined_image", "mask_L", "mask_R", "combined_mask")
    CATEGORY = "image/face"
    FUNCTION = "process"

    def process(self, image_L, mask_L, image_R, mask_R, mode, background,
               move_L_x, move_L_y, move_R_x, move_R_y, 
               size_L, size_R, angle_L, angle_R,
               detection_method="opencv", custom_width=760, custom_height=1024):
        try:
            with Timer("总处理时间"):
                # 保存当前模式和检测方法，供其他方法使用
                ...
                mask_canvas_L = np.zeros((target_h, target_w), dtype=np.uint8)
                mask_canvas_R = np.zeros((target_h, target_w), dtype=np.uint8)
                
                # 处理左侧人物
                with Timer("处理左侧人物"):
                    np_face_L, np_mask_L = self._prepare_inputs(image_L, mask_L)
                    face_center_L = self._detect_face_center(np_face_L, np_mask_L)
                    face_info_L = self._analyze_face(np_face_L, np_mask_L)
                    
                    # 获取人物比例参数
                    params_L = self._get_person_params(mode, face_info_L)
                    
                    # 智能缩放
                    scaled_face_L, scaled_mask_L, face_center_scaled_L = self._smart_scaling(
                        np_face_L, np_mask_L, target_h, params_L["head_ratio"], size_L, face_center_L, face_info_L
                    )
                    
                    # 修改流程：先旋转，后定位
                    # 应用旋转
                    if angle_L != 0:
                        rotated_face_L, rotated_mask_L, rotated_center_L = self._rotate_image_improved(
                            scaled_face_L, scaled_mask_L, angle_L, bg_color, face_center_scaled_L
                        )
                    else:
                        rotated_face_L, rotated_mask_L, rotated_center_L = scaled_face_L, scaled_mask_L, face_center_scaled_L
                    
                    # 应用定位
                    final_face_L, final_mask_L, final_face_center_L = self._apply_positioning(
                        rotated_face_L, rotated_mask_L, target_w, target_h, bg_color, 
                        move_L_x, move_L_y, rotated_center_L, "left"
                    )
                
                # 处理右侧人物
                with Timer("处理右侧人物"):
                    np_face_R, np_mask_R = self._prepare_inputs(image_R, mask_R)
                    face_center_R = self._detect_face_center(np_face_R, np_mask_R)
                    face_info_R = self._analyze_face(np_face_R, np_mask_R)
                    
                    # 获取人物比例参数
                    params_R = self._get_person_params(mode, face_info_R)
                    
                    # 智能缩放
                    scaled_face_R, scaled_mask_R, face_center_scaled_R = self._smart_scaling(
                        np_face_R, np_mask_R, target_h, params_R["head_ratio"], size_R, face_center_R, face_info_R
                    )
                    
                    # 修改流程：先旋转，后定位
                    # 应用旋转
                    if angle_R != 0:
                        rotated_face_R, rotated_mask_R, rotated_center_R = self._rotate_image_improved(
                            scaled_face_R, scaled_mask_R, angle_R, bg_color, face_center_scaled_R
                        )
                    else:
                        rotated_face_R, rotated_mask_R, rotated_center_R = scaled_face_R, scaled_mask_R, face_center_scaled_R
                    
                    # 应用定位
                    final_face_R, final_mask_R, final_face_center_R = self._apply_positioning(
                        rotated_face_R, rotated_mask_R, target_w, target_h, bg_color, 
                        move_R_x, move_R_y, rotated_center_R, "right"
                    )
                
                # 合成最终图像
                with Timer("合成最终图像"):
                    combined_image, mask_L_out, mask_R_out, combined_mask = self._combine_images(
                        final_face_L, final_mask_L, final_face_R, final_mask_R, 
                        target_w, target_h, bg_color
                    )
                    
                    # 检查人物是否脱离画布
                    self._check_boundary(final_mask_L, "左侧人物")
                    self._check_boundary(final_mask_R, "右侧人物")
                
                return self._wrap_outputs(combined_image, mask_L_out, mask_R_out, combined_mask)
            ...
            # 创建空白图像作为输出
            blank = np.full_like(image_L[0].cpu().numpy(), 0.0)
            blank_tensor = torch.from_numpy(blank).unsqueeze(0)
            return (blank_tensor, mask_L, mask_R, mask_L)

    # 工具方法 --------------------------------------------------
    def _calculate_dimensions(self, mode, custom_width, custom_height) -> Tuple[int, int]:
        """智能尺寸计算系统"""
        mode_params = {
            "half_body": (760, 1024),    # 半身模式
            "full_body": (800, 1200),    # 全身模式 - 调整为更高的画布以容纳全身
    ...
    def _analyze_face(self, image, mask) -> Dict[str, Any]:
        ...
        with Timer("人脸分析"):
            ...
            # 检查缓存
            if FaceAutoFitDouble._last_image_hash == image_hash and FaceAutoFitDouble._last_face_info is not None:
                logger.info("使用缓存的人脸分析结果")
                return FaceAutoFitDouble._last_face_info.copy()
            ...
            # 更新缓存
            FaceAutoFitDouble._last_image_hash = image_hash
            FaceAutoFitDouble._last_face_info = face_info.copy()
    ...
    def _analyze_face_opencv(self, image, face_info):
        ...
        try:
            # 延迟加载人脸检测器
            if FaceAutoFitDouble._face_cascade is None:
                FaceAutoFitDouble._face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
                
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            faces = FaceAutoFitDouble._face_cascade.detectMultiScale(gray, 1.1, 4)
    ...
    def _analyze_face_mediapipe(self, image, face_info):
        ...
        try:
            ...
            if FaceAutoFitDouble._mediapipe_face_mesh is None:
                mp_face_mesh = mp.solutions.face_mesh
                FaceAutoFitDouble._mediapipe_face_mesh = mp_face_mesh.FaceMesh(
            ...
            # 处理图像
            mp_results = FaceAutoFitDouble._mediapipe_face_mesh.process(rgb_image)
            ...
            if mp_results.multi_face_landmarks:
                ...
                right_eye_x = sum(landmarks[idx].y for idx in RIGHT_EYE_INDICES) / len(RIGHT_EYE_INDICES) * h
    ...
    def _analyze_face_dlib(self, image, face_info):
        ...
        try:
            ...
            # 延迟加载dlib模型
            if FaceAutoFitDouble._dlib_detector is None:
                FaceAutoFitDouble._dlib_detector = dlib.get_frontal_face_detector()
                
            if FaceAutoFitDouble._dlib_predictor is None:
                FaceAutoFitDouble._dlib_predictor = dlib.shape_predictor(predictor_path)
            ...
            faces = FaceAutoFitDouble._dlib_detector(gray, 0)
            
            if len(faces) > 0:
                # 获取人脸关键点
                shape = FaceAutoFitDouble._dlib_predictor(gray, faces[0])
    ...
    def _get_person_params(self, mode, face_info) -> Dict[str, float]:
        ...
        if mode == "half_body":
            ...
            # 全身模式 - 参考图2调整比例
            params["head_ratio"] = 0.08    # 减小头部比例，从0.10调整为0.08
            params["eye_position_ratio"] = 0.12  # 将眼睛位置调高，约为画面高度的12%处
    ...
    def _detect_face_center(self, image, mask) -> Tuple[int, int]:
        """检测人脸中心点 - 优化版"""
        # 检查是否有缓存的人脸信息
        if FaceAutoFitDouble._last_face_info is not None and FaceAutoFitDouble._last_face_info["eyes_center"] is not None:
            return FaceAutoFitDouble._last_face_info["eyes_center"]
    ...
    def _rotate_image_improved(self, image, mask, angle, bg_color, center) -> Tuple[np.ndarray, np.ndarray, Tuple[int, int]]:
        """改进的旋转函数 - 支持扩展画布以避免裁剪"""
        if angle == 0:
        ...
        # 返回旋转后的图像、蒙版和新的中心点坐标
        return rotated_img, rotated_mask, (new_cx, new_cy)
    
    def _rotate_image(self, image, mask, angle, bg_color, center) -> Tuple[np.ndarray, np.ndarray]:
        """以指定中心点旋转图像"""
        if angle == 0:
            return image, mask
    ...
    def _apply_positioning(self, image, mask, target_w, target_h, bg_color, move_x, move_y, face_center, position) -> Tuple[np.ndarray, np.ndarray, Tuple[int, int]]:
        """精确定位系统 - 根据眼睛位置和用户调整"""
        # 创建画布
        canvas = np.full((target_h, target_w, 3), bg_color, dtype=np.uint8)
        ...
        # 根据当前模式设置眼睛位置比例
        if self.current_mode == "half_body":
            eye_position_ratio = 0.25  # 半身模式眼睛位置
        elif self.current_mode == "full_body":
            eye_position_ratio = 0.15  # 全身模式眼睛位置
        else:
            eye_position_ratio = 0.30  # 默认/自定义模式
        
        # 计算水平位置 - 左侧人物靠左，右侧人物靠右
        if position == "left":
            # 左侧人物 - 眼睛在画布宽度的1/3处
            A_x = int(target_w * 0.33)
        else:
            # 右侧人物 - 眼睛在画布宽度的2/3处
            A_x = int(target_w * 0.67)
    ...
    def _combine_images(self, image_L, mask_L, image_R, mask_R, target_w, target_h, bg_color) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """合成最终图像 - 先绘制左侧人物（后层），再绘制右侧人物（前层）"""
        # 创建最终画布
        final_canvas = np.full((target_h, target_w, 3), bg_color, dtype=np.uint8)
        combined_mask = np.zeros((target_h, target_w), dtype=np.uint8)
        
        # 先绘制左侧人物（后层）
        mask_L_3d = np.expand_dims(mask_L, axis=2) / 255.0
        final_canvas = image_L * mask_L_3d + final_canvas * (1 - mask_L_3d)
        combined_mask = np.maximum(combined_mask, mask_L)
        
        # 再绘制右侧人物（前层）
        mask_R_3d = np.expand_dims(mask_R, axis=2) / 255.0
        final_canvas = image_R * mask_R_3d + final_canvas * (1 - mask_R_3d)
        combined_mask = np.maximum(combined_mask, mask_R)
        
        return final_canvas.astype(np.uint8), mask_L, mask_R, combined_mask
    ...
    def _wrap_outputs(self, image, mask_L, mask_R, mask_combined) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """输出格式转换"""
        return (
            torch.from_numpy(image.astype(np.float32)/255.0).unsqueeze(0),
            torch.from_numpy(mask_L.astype(np.float32)/255.0).unsqueeze(0),
            torch.from_numpy(mask_R.astype(np.float32)/255.0).unsqueeze(0),
            torch.from_numpy(mask_combined.astype(np.float32)/255.0).unsqueeze(0)
        )
# 将这两行移到类外部
